...
async function bootstrap() {
  ...
  // Placeholder: payment intent + webhook endpoints
  app.post('/v1/invoices/:id/intent', (req, res) => res.status(501).json({ error: 'not_implemented', id: req.params.id }));
  app.post('/v1/webhooks/razorpay', (_req, res) => res.status(501).json({ error: 'not_implemented' }));
  ...
  const server = app.listen(PORT, () => logger.info('Payment service started', { port: PORT }));
