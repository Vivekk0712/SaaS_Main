...
async function bootstrap() {
  ...
  // Placeholder send API
  app.post('/v1/notifications/send', (_req, res) => res.status(501).json({ error: 'not_implemented' }));
  ...
  const server = app.listen(PORT, () => logger.info('Notification service started', { port: PORT }));
