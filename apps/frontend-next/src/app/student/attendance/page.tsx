      ...
      <div className="dash-wrap student-main">
        <div className="dash-layout">
          ...
          <div className="dash dash-wide">
            ...
            <p className="subtitle">Subject-wise summary plus monthly charts and detailed logs.</p>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11 }}>
                        ...
                        <button
                          ...
                          style={{
                            padding: '2px 6px',
                            borderRadius: 999,
                            opacity: monthlyIndex < monthly.length - 1 ? 1 : 0.4,
                          }}
                          disabled={monthlyIndex >= monthly.length - 1}
                          onClick={() =>
                            setMonthlyIndex(i => Math.min(monthly.length - 1, i + 1))
                          }
                        >
                          ▶
                        </button>
                ...
                {(subjects.length ? subjects : Object.keys(stats)).map((sub) => {
                  ...
                  return (
                    <div key={key} className="subject-card">
                      <div
                        className={`subject-header banner-${tag}`}
                        style={{
                          display: 'grid',
                          ...
                          alignItems: 'center',
                          padding: '12px 14px',
                          borderBottom: opened ? '1px solid var(--panel-border)' : 'none'
                        }}
                      >
                        <div style={{ fontWeight: 800, fontSize: 16 }}>{key}</div>
                        ...
                        <button
                          type="button"
                          onClick={() => setOpen(prev => ({ ...prev, [key]: !opened }))}
                      ...
                      {opened && (
                        <div className="subject-log">
                          <div
                            style={{
                              display: 'grid',
                              gridTemplateColumns: '1.2fr 0.8fr 0.8fr 2fr',
                              gap: 6,
                              padding: '8px 10px',
                            ...
                            {s.logs.map((entry, idx) => (
                              ...
                              >
                                ...
                                <span
                                  style={{
                                    fontWeight: 700,
                                    color: entry.status === 'Present' ? 'var(--success)' : entry.status === 'Leave' ? '#f97316' : 'var(--danger)'
                                  }}
                                >
                                  {entry.status}
                                </span>
                ...
                <div className="card" style={{ padding: '10px 12px', borderRadius: 16 }}>
                  ...
                  {Object.keys(daily).length === 0 && (
                    <div className="note" style={{ marginTop: 6 }}>
                      No attendance recorded yet. Snapshot will appear once classes are marked.
                    </div>
                  )}
                          ...
                          <>
                            ...
                            <div style={{ display: 'grid', gap: 4 }}>
                              {day.items.map((it, idx) => (
                                ...
                                >
                                  <span style={{ fontSize: 11, opacity: 0.7 }}>Hour {it.hour}</span>
                                  <span
                                    style={{
                                      fontSize: 12,
                                      fontWeight: 700,
