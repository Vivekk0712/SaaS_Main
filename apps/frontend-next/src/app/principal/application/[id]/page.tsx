...
export default function PrincipalApplicationDetail() {
  ...
  const setField = (path: string, value: any) => {
    setDraft((d: any) => {
      ...
      let cur: any = next
      for (let i = 0; i < keys.length - 1; i++) { const k = keys[i]; cur[k] = cur[k] || {}; cur = cur[k] }
  ...
  const saveEdits = async () => {
    ...
    try {
      setSaving(true)
      try {
        const r = await fetch(`${API}/v1/onboarding/staff/applications/${id}`, { method: 'PATCH', headers: { 'content-type': 'application/json', 'x-role': 'principal', 'x-password': '12345' }, body: JSON.stringify({ application: draft }) })
  ...
  const confirm = async () => {
    try {
      setSaving(true)
      try {
        const r = await fetch(`${API}/v1/onboarding/staff/applications/${id}/confirm`, { method: 'POST', headers: { 'x-role': 'principal', 'x-password': '12345' } })
        if (!r.ok) throw new Error('remote_err')
      } catch {
  ...
  const saveFees = async () => {
    try {
      setSaving(true)
      try {
        const r = await fetch(`${API}/v1/onboarding/staff/applications/${id}/fees`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-role': 'principal', 'x-password': '12345' }, body: JSON.stringify({ items, total, installments: instParts.map((a,i)=>({ label: `Installment ${i+1}`, amount: a, dueDate: instDue[i]||null })) }) })
        if (!r.ok) throw new Error('remote_err')
      } catch {
  ...
  return (
    <div className="container">
      <div className="card" style={{ maxWidth: 980 }}>
        <h1 className="title">Application Review (Principal)</h1>
        ...
        <div style={{ marginTop: 24 }}>
            ...
            <tbody>
              {items.map((it, i) => (
                <tr key={i} style={{ borderTop: '1px solid #eee' }}>
                  <td><input className="input" value={it.label} onChange={e=>upd(i,{label:e.target.value})} placeholder="Tuition / Transport / Misc" /></td>
                  <td align="right"><input className="input" value={it.amount} onChange={e=>upd(i,{amount:Number(e.target.value||0)})} /></td>
                  <td><button className="btn-ghost" onClick={() => rmRow(i)} type="button">Remove</button></td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="actions">
            <button className="btn" type="button" disabled={saving} onClick={saveFees}>{saving? 'Saving…' : 'Save Fee'}</button>
          </div>

          <div className="actions" style={{ justifyContent:'space-between' }}>
            <div className="field" style={{ maxWidth: 280 }}>
              <span className="label">Installments (count)</span>
