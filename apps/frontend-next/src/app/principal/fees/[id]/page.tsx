...
export default function PrincipalFeeEditPage() {
  ...
  const load = async () => {
    try {
      ...
      let j: any = null
      try {
        const r = await fetch(`${API}/v1/onboarding/staff/fees`, { headers: { 'x-role': 'principal', 'x-password': '12345' } })
        j = await r.json()
      } catch {
        ...
        {loading ? <p className="subtitle">Loading…</p> : (
          <>
            ...
            <div className="actions">
              ...
              <button className="btn" type="button" onClick={async()=>{ await fetch(`/api/local/staff/applications/${id}/fees/confirm`, { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ role:'principal' }) }); alert('Fees confirmed by Principal') }}>Confirm Fees</button>
            </div>
            <div className="actions" style={{ justifyContent:'space-between' }}>
